...
describes what users get. Pick a format with `-o`, or let catfact choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
catfact <command> -o table   # aligned columns for reading
catfact <command> -o jsonl   # one JSON object per line, for piping
catfact <command> -o json    # a single JSON array
catfact <command> -o csv     # spreadsheet friendly
catfact <command> -o tsv     # tab-separated
catfact <command> -o url     # just the URL column
catfact <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
catfact <command> --fields id,title,url
```
...
```bash
catfact <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
catfact <command>            # a table, because this is a terminal
catfact <command> | wc -l    # JSONL, because this is a pipe
```
