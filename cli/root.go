// Package cli assembles the catfact command tree from the catfact
...
// NewApp assembles the kit application from the catfact domain. The
...
//
// To add a command, declare it in catfact/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := catfact.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(catfact.Domain{}).Register(app)
